...
def max_possible_damage(occurrences, deck_size_list=None) -> int:
    trigger_bonus = {None: 0, "single": 2, "twin": 4}
    base = sum(draw_count + trigger_bonus[trigger_type]
               for draw_count, trigger_type in parse_occurrences(occurrences))
...
def generate_table(deck_size_list, climax_list, occurrences, n_trials,
    ...
    occurrences        : damage sequence to test
    ...
    max_damage_column = (
        max_possible_damage(occurrences, deck_size_list)
